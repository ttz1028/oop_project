...
public:
    ...
    Book* All_books_list();  //返回书的数组
    int* All_books_number_list();  //返回所有不同中书的数量的数组
    void read_book_record(string filename);  //从txt中读取文件    
    int get_number_of_books_in_labrary(); //返回图书馆现有多少本书
    ~Library();
};
